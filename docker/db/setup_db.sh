...
docker exec database_master sh -c "mysql -u root -pPaletteSecret -e '$sql_slave_user'"
MS_STATUS=`docker exec database_master sh -c 'mysql -u root -pPaletteSecret -e "SHOW MASTER STATUS"'`
...
start_slave_cmd='mysql -u root -pPaletteSecret -e "'
start_slave_cmd+="$sql_set_master"
start_slave_cmd+='"'
docker exec database_slave sh -c "$start_slave_cmd"
docker exec database_slave sh -c "mysql -u root -pPaletteSecret -e 'SHOW SLAVE STATUS \G'"